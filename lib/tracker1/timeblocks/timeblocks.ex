defmodule Tracker1.Timeblocks do
  @moduledoc """
  ...
  def calc_time_taskid(task_id) do
    query = from(t in Timeblock, group_by: t.task, select: sum(t.start))
    start = Repo.one(query)
    query = from(t in Timeblock, group_by: t.task, select: sum(t.end))
    finish = Repo.one(query)
  ...
  def get_time_timeblock_by_taskid(taskid) do
    query = Repo.all from t in Timeblock,
